...
RegisterNetEvent('seatbelt:client:ToggleCruise', function(cruiseSpeed) -- Triggered in smallresources
    cruiseOn = cruiseSpeed
end)
            ...
            if IsPedInAnyVehicle(ped, false) then
               -- hasHarness()
                ...
                if seatbeltOn ~= true and IsThisModelACar(veh) then
                    TriggerEvent("InteractSound_CL:PlayOnOne", "beltalarm", 0.3)
                end
                Wait(5000)
            end
        end
            ...
            if weapon ~= `WEAPON_UNARMED` then
                if IsPedShooting(ped) and not IsWhitelistedWeaponStress(weapon) then
                    if math.random() < config.StressChance then
                        TriggerServerEvent('hud:server:GainStress', math.random(3, 8))
                    end
                    Wait(100)
                else
                    Wait(500)
